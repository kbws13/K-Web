...
// NewKEnv 有一个参数，.env文件所在的目录
// example: NewKEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewKEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewKEnv param error")
	}
...
func (en *KEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *KEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *KEnv) Get(key string) string {
...
func (en *KEnv) All() map[string]string {
	return en.maps
}

...
type devConfig struct {
	Port    string   // 调试模式最终检讨的短裤，默认为8070
	Backend struct { // 后端调试模式配置
		RefreshTime   int    // 调试模式后端更新时间，如果文件变更，等待3s才进行一次更新，能让频繁保存变更更为流畅，默认1s
		Port          string // 后端监听端口，默认8072
		MonitorFolder string // 监听文件夹，默认为 AppFolder
	}
...
func (p *Proxy) rebuildBackend() error {
	// 重新编译K-Web
	cmdBuild := exec.Command("./K-Web", "build", "backend")
...
func (p *Proxy) restartBackend() error {
	...
	// 杀死之前的进程
	if p.backendPid != 0 {
		pro, err := os.FindProcess(p.backendPid)
		if err != nil {
			return err
		}
		err = pro.Kill()
	...
	// 使用命令行启动后端进程
	cmd := exec.Command("./"+outputFile, "app", "start", "--address="+kAddress)
